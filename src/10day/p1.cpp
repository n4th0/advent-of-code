...
int prueba(int i, int j, int counter, vector<Coord> &m){
    ...
    // cout <<"from pathing "<<i+1<<" "<<j+1<<" " <<counter<<" " <<matrix[i][j]<<endl;
        counter++;
        if (i + 1 < matrix[0].size()) { prueba(i+1, j,  counter, m); }
        if (j + 1 < matrix[0].size()) {  prueba(i, j+1,  counter, m); }
        if (j - 1 >= 0) { prueba(i, j-1,  counter, m); }
        if (i - 1 >= 0) { prueba(i-1, j,  counter, m); }
    }
